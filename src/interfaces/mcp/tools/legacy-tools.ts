 ...
 * Tools: 29 tools across 8 categories (knowledge, mailbox, identity, skills, terminal-docs, terminal-status, system, project-automation)
 */
...
import {
  ...
  handleInitializeProjectTerminals,
  handleGenerateProjectTmuxScript,
  handleCalculateWoodworkingCost,
  handleCalculateMaterialAllowance,
  handleCheckTechnicalChecklist,
  SelectActiveProjectArgs,
  CalculateWoodworkingCostArgs,
  CalculateMaterialAllowanceArgs,
  CheckTechnicalChecklistArgs,
} from '../../../woodworkingTools';
import {
  createGoal,
  listGoals,
  getGoal,
...
const TOOLS = [
  ...
  // ─── Woodworking & Orchestrator Tools ───────────────────────────────────────
  {
    name: 'select_active_project',
    description: 'Set the current active project directory. This redirects all terminals and environment parameters to this workspace.',
    inputSchema: {
      type: 'object',
      properties: {
        project_path: {
          type: 'string',
          description: 'Absolute path to the project directory (e.g. C:/Users/szant/Documents/Development/Cabinet_bilder_scripts)',
        },
      },
  ...
  {
    name: 'initialize_project_terminals',
    description: 'Create the terminals mailbox folders (inbox, outbox, archive) and CLAUDE.md files inside the active project directory.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  {
    name: 'generate_project_tmux_script',
    description: 'Generate start-project-tmux.sh shell script in the active project directory to orchestrate the terminal fleet.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  {
    name: 'calculate_woodworking_cost',
    description: 'Calculate comprehensive production costs, net price, rounded price, and gross price using the 11-step formula (page 40-41).',
    inputSchema: {
      type: 'object',
      properties: {
        material_cost: {
          type: 'number',
          description: 'Net cost of all raw and auxiliary materials (HUF)',
        },
        labor_hours: {
          type: 'number',
          description: 'Estimated hours of labor',
        },
        hourly_rate: {
          type: 'number',
          description: 'Net hourly wage rate in HUF (default: 5000)',
        },
        other_costs: {
          type: 'number',
          description: 'Net cost of other direct components or third-party work (default: 0)',
        },
        overhead_percent: {
          type: 'number',
          description: 'Overhead percent of direct costs (default: 20)',
        },
        profit_percent: {
          type: 'number',
          description: 'Profit margin percent of self-cost (default: 15)',
        },
        vat_percent: {
          type: 'number',
          description: 'Value Added Tax (ÁFA) percentage (default: 27)',
        },
      },
  ...
  {
    name: 'calculate_material_allowance',
    description: 'Calculate rough cutting size allowances (gross dimensions) and material requirements (m³ volume for solid wood, m² area for panels) for parts.',
    inputSchema: {
      type: 'object',
      properties: {
        elements: {
          ...
          items: {
            ...
            properties: {
              ...
              length: { type: 'number', description: 'Net length in mm' },
              width: { type: 'number', description: 'Net width in mm' },
              thickness: { type: 'number', description: 'Net thickness in mm' },
              quantity: { type: 'number', description: 'Quantity needed' },
              type: {
                type: 'string',
                enum: ['solid_wood', 'panel_product'],
                description: 'Type of material: solid_wood (tömörfa) or panel_product (laptermék)',
              },
            },
            required: ['name', 'length', 'width', 'thickness', 'quantity', 'type'],
          },
        },
      },
      required: ['elements'],
  ...
  {
    name: 'check_technical_checklist',
    description: 'Verify if the essential production preparation documents exist in the design directory.',
    inputSchema: {
      type: 'object',
      properties: {
        project_dir: {
          type: 'string',
          description: 'Path to the design folder (e.g. G:/Saját meghajtó/Designs/Tamási Gábor/114 - Pizzéria)',
        },
      },
...
async function handleToolCall(
  ...
  try {
    switch (name) {
      // Woodworking & Orchestration tools
      case 'select_active_project': {
        const result = await handleSelectActiveProject(args as any);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      case 'initialize_project_terminals': {
        const result = await handleInitializeProjectTerminals();
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      case 'generate_project_tmux_script': {
        const result = await handleGenerateProjectTmuxScript();
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      case 'calculate_woodworking_cost': {
        const result = await handleCalculateWoodworkingCost(args as any);
        return { content: [{ type: 'text', text: result.markdown || JSON.stringify(result, null, 2) }] };
      }
      case 'calculate_material_allowance': {
        const result = await handleCalculateMaterialAllowance(args as any);
        return { content: [{ type: 'text', text: result.markdown || JSON.stringify(result, null, 2) }] };
      }
      case 'check_technical_checklist': {
        const result = await handleCheckTechnicalChecklist(args as any);
        return { content: [{ type: 'text', text: result.markdown || JSON.stringify(result, null, 2) }] };
      }
      ...
      case 'read_inbox_message': {
        ...
        if (callerTerminal && callerTerminal !== 'root' && callerTerminal !== terminal) {
          return {
            content: [{ type: 'text', text: JSON.stringify({
              success: false,
...
`;
        ...
        const TERMINALS_ROOT = '/opt/spaceos/terminals';
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                queue: focus.queue.map((item: any) => ({
                  id: item.id,
      ...
      case 'spawn_parallel_workers': {
        ...
        const results = await Promise.all(
          firstBatch.map(async (taskId: any) => {
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                activeCount: workers.filter((w: any) => w.status === 'running').length,
                queuedCount: workers.filter((w: any) => w.status === 'queued').length,
                currentHourlyCost,
                maxParallel,
                alertLevel: costAlert, // Dashboard uses alertLevel
                capacity: {
                  current: workers.filter((w: any) => w.status === 'running').length,
                  max: maxParallel,
                  available: Math.max(0, maxParallel - workers.filter((w: any) => w.status === 'running').length),
                },
              }, null, 2),
            },
          ],
      ...
      case 'trigger_goal': {
        ...
        if (callerTerminal && callerTerminal !== 'root' && callerTerminal !== 'monitor') {
          return {
            content: [{ type: 'text', text: JSON.stringify({
              success: false,
        ...
        const triggerResult = await createTask({
          ...
          description: goal.on_complete.prompt
            ...
            .replace(/\{\{completed_criteria\}\}/g, results.map((r: any) => `- ${r.met ? '✓' : '✗'} ${r.criterion.type}: ${r.details}`).join('\n')),
          priority: 'high',
          model: 'sonnet',
          ref: goalId,
        });
...
import { toolRegistry, ToolDefinition, ToolHandler } from './base-tool';

export function registerLegacyTools(): void {
  for (const tool of TOOLS) {
    ...
    const handler: ToolHandler = async (args, context) => {
      const callerTerminal = context.terminal || 'root';
      const rawResult = await handleToolCall(tool.name, args, callerTerminal);
      return {
  ...
  console.log(`[MCP] ⚙️ Registered ${TOOLS.length} legacy tools in global toolRegistry`);
}
