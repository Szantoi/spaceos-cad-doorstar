...
router.get('/fetch/:terminal/:messageId', requireTerminalAuth, async (req: Request, res: Response) => {
  try {
    ...
    const inboxPath = path.join(process.env.TERMINALS_PATH || '/opt/spaceos/terminals', terminal, 'inbox');
...
router.post('/sync', async (req: Request, res: Response) => {
  try {
    const path = await import('path');
    const epicsPath = req.body.path || path.join(process.env.SPACEOS_ROOT || '/opt/spaceos', 'docs/projects/EPICS.yaml');
