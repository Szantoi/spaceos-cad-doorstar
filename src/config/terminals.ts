...
export function loadConfig(): TerminalsConfig {
  ...
  try {
    ...
    if (process.env.TERMINALS_PATH) {
      _config.terminalsRoot = process.env.TERMINALS_PATH;
    } else if (process.env.SPACEOS_ROOT) {
      _config.terminalsRoot = path.join(process.env.SPACEOS_ROOT, 'terminals');
    }

    if (process.env.TMUX_SOCKET) {
      _config.tmuxSocket = process.env.TMUX_SOCKET;
    }

    console.log(`[Config] Loaded ${Object.keys(_config.terminals).length} terminals from config (terminalsRoot: ${_config.terminalsRoot})`);
    return _config;
  } catch (err) {
    console.error(`[Config] Failed to load terminals.json: ${err}`);
    // Fallback to minimal config
    const rootPath = process.env.TERMINALS_PATH || 
      (process.env.SPACEOS_ROOT ? path.join(process.env.SPACEOS_ROOT, 'terminals') : '/opt/spaceos/terminals');
    _config = {
      terminalsRoot: rootPath,
      terminals: {
      ...
      legacyAliases: {},
      tmuxSocket: process.env.TMUX_SOCKET || '/tmp/spaceos.tmux',
    };
    return _config;
  }
}
