...
const TERMINALS_ROOT = process.env.TERMINALS_PATH || path.join(REPO_ROOT, 'terminals');
// Legacy mailbox root (symlink for backward compatibility)
const LEGACY_MAILBOX_ROOT = process.env.SPACEOS_ROOT ? path.join(process.env.SPACEOS_ROOT, 'docs/mailbox') : path.join(REPO_ROOT, 'docs/mailbox');
const TASKS_ROOT = process.env.SPACEOS_ROOT ? path.join(process.env.SPACEOS_ROOT, 'docs/tasks') : path.join(REPO_ROOT, 'docs/tasks');
